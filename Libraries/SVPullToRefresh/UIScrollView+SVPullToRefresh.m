...
static CGFloat const SVPullToRefreshViewHeight = 80.0;
...
void * const kDQPullToRefreshYOriginOffsetKey = (void *)&kDQPullToRefreshYOriginOffsetKey;

@implementation UIScrollView (SVPullToRefresh)

@dynamic pullToRefreshView, showsPullToRefresh, dq_pullToRefreshYOriginOffset;

- (CGFloat)dq_yOriginForPosition:(SVPullToRefreshPosition)position
{
    CGFloat yOrigin = 0.0;
    switch (position)
    {
        case SVPullToRefreshPositionTop:
            yOrigin = -SVPullToRefreshViewHeight + self.dq_pullToRefreshYOriginOffset;
            break;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    CGFloat yOrigin = [self dq_yOriginForPosition:position];
    CGRect frame = CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
//    NSLog(@"frame: %@", NSStringFromCGRect(frame));
//    NSLog(@"y offset: %@", @(self.dq_pullToRefreshYOriginOffset));
    if(self.pullToRefreshView) {
        self.pullToRefreshView.frame = frame;
        self.pullToRefreshView.originalTopInset = self.contentInset.top;
//        NSLog(@"originalTopInset: %f", self.pullToRefreshView.originalTopInset);
//        NSLog(@"originalBottomInset: %f", self.pullToRefreshView.originalBottomInset);
        ...
        view.arrowColor = [UIColor dq_phoneGrayTextColor];
        view.textColor = [UIColor dq_phoneGrayTextColor];
...
- (void)setDq_pullToRefreshYOriginOffset:(CGFloat)dq_pullToRefreshYOriginOffset
{
//    NSLog(@"setting y offset to: %@", @(dq_pullToRefreshYOriginOffset));
    objc_setAssociatedObject(self, kDQPullToRefreshYOriginOffsetKey,
                             @(dq_pullToRefreshYOriginOffset),
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGFloat)dq_pullToRefreshYOriginOffset
{
    NSNumber *n = objc_getAssociatedObject(self, kDQPullToRefreshYOriginOffsetKey);
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:DQLocalizedString(@"Pull to refresh...", @"Prompt to pull down on the page to refresh content"),
                             DQLocalizedString(@"Release to refresh...", @"Prompt to release the page to refresh content"),
                             DQLocalizedString(@"Loading...", @"Data is being loaded from server indicator label"),
                                nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                yOrigin = -SVPullToRefreshViewHeight + self.scrollView.dq_pullToRefreshYOriginOffset;
                break;
            case SVPullToRefreshPositionBottom:
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        switch (self.position) {
            case SVPullToRefreshPositionTop:
                scrollOffsetThreshold = self.frame.origin.y - self.originalTopInset - self.scrollView.dq_pullToRefreshYOriginOffset;
                break;
            case SVPullToRefreshPositionBottom:
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = DQLocalizedString(@"Pull to refresh...", @"Prompt to pull down on the page to refresh content");
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    self.dateLabel.text = [NSString stringWithFormat:DQLocalizedString(@"Last Updated: %@", @"Date the item was last updated"), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:DQLocalizedString(@"Never", @"Indicating an item has never been updated")];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:DQLocalizedString(@"Last Updated: %@", @"Date the item was last updated"), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:DQLocalizedString(@"Never", @"Indicating an item has never been updated")];
}
...
- (void)setOriginalContentInset:(UIEdgeInsets)contentInset
{
    self.originalTopInset = contentInset.top;
}
